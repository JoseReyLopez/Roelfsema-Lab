...
from lucent.modelzoo import util, vgg19  #inceptionv1
...
assert int(args.layer) in [0,2,5,7,10,12,14,16,19,21,23,25,28,30,32,34], 'Layer options: 0,2,5,7,10,12,14,16,19,21,23,25,28,30,32,34'
layer   = 'features.'+str(args.layer)
layers  = ['features.' + str(args.layer)]
print(roi_name)
...
if os.name == 'nt':
    ...
    snapshot_path = f'E:/Jose/snapshots/array'+ roi_name +'_neural_model_'+date+'.pt'
    loss_plot_path = f'E:/Jose/training_data/training_loss_classifier_{current_datetime}.png'


if os.name == 'posix':

    os.chdir('/home/jose/Desktop/lucent-things/Results_Bashivan_Vgg')
...
#    snapshot_path  = '/home/jose/Desktop/snapshots/array'+roi_name+'_neural_model_'+date+'.pt'
    results_save   = '/home/jose/Desktop/lucent-things/Results_Bashivan_Vgg'
    loss_plot_path = '/home/jose/Desktop/training_data/training_loss_classifier_{current_datetime}.png'



create_folder(args.roi, args.layer)
...
if os.name == 'nt':
    snapshot_pattern = f'E:/Jose/snapshots/neural_model_{backbone}_{args.layer}.pt'
if os.name == 'posix':
    snapshot_pattern  = f'/home/jose/Desktop/snapshots/neural_model_{backbone}_{args.layer}.pt'
...
# optimizer and lr scheduler
optimizer = torch.optim.Adam([net.w_s,net.w_f],lr=learning_rate,weight_decay=weight_decay)
...
for epoch in range(nb_epochs):
    ...
    # get the inputs & wrap them in tensor
    batch_idx = np.random.choice(np.linspace(0,train_data.shape[0]-1,train_data.shape[0]), size=batch_size,replace=False).astype('int')
    ...
    loss = criterion(outputs, neural_batch.float()) + smoothing_laplacian_loss(net.w_s, torch.device("cuda:0" if GPU else "cpu"), 
                                                      weight=smooth_weight) \
                                                    + sparse_weight * torch.norm(net.w_f,1)
    ...
    val_img_data = val_img_data.cuda()
    ...
    if epoch % save_epochs == save_epochs - 1:
        ...
        torch.save(net.state_dict(), f'Bashivan_Vgg_{args.roi}_{args.layer}_epoch_{int(no_epoch)}_loss_{val_loss}_smoothweight_{args.smoothweights}_sparseweight_{args.sparseweights}_weightdecay_{args.weightdecays}.pt')
        delete_all_but_lowest_validation_file()

# save the weights, we're done!

#os.makedirs(os.path.dirname(snapshot_path), exist_ok=True)
#torch.save(net.state_dict(), '/home/jose/Desktop/lucent-things/Results_Bashivan_Vgg/saved_net.pt')