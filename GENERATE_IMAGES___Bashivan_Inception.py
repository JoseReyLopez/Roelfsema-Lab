...
if not os.path.exists('Bashivan_Inception'):
    os.mkdir('Bashivan_Inception')

os.chdir('Bashivan_Inception')



net = BashivanInception(args.roi, args.layer)
...
np.save(f'Bashivan_Inception_roi_{args.roi}_layer_{args.layer}', img[0])
f, ax = plt.subplots(1, 5, figsize = (45, 10))
for i in range(5):
    ax[i].imshow(img[0][i, :])
    ax[i].set_axis_off()

plt.savefig(f'Bashivan_Inception_Img_roi_{args.roi}_layer_{args.layer}.png', dpi = 150)
